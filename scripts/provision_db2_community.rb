...
def provision_db2_community(root_loc)
  ...
  config['applications'].each do |appname, _appconfig|
    ...
    if File.exist?("#{root_loc}/apps/#{appname}/fragments/db2-community-init-fragment.sql")
      database_initialised = process_db2_community_fragment(root_loc, appname, database_initialised)
    else
...
def process_db2_community_fragment(root_loc, appname, database_initialised)
  result = database_initialised
  puts colorize_pink("Found some in #{appname}")
  if commodity_provisioned?(root_loc, appname, 'db2_community')
